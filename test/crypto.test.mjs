...
const canonRoot = process.env.OLP_CANON_PATH
  ? pathToFileURL(`${resolve(process.env.OLP_CANON_PATH)}${sep}`)
  : new URL("../../olp-wire-canon/", import.meta.url);
const canonFile = (path) => new URL(path, canonRoot);
...
test("Node verifies every Python Canon receipt vector", async () => {
  ...
  for (const name of names) {
    const path = canonFile(`vectors/valid/${name}`);
...
test("Node validates Python disclosures and rejects altered disclosures", async () => {
  const receipt = JSON.parse(await readFile(
    canonFile("vectors/valid/coherence-input-receipt.json"),
    "utf8",
  ));
  const valid = JSON.parse(await readFile(
    canonFile("vectors/valid/coherence-input-disclosure.json"),
    "utf8",
  ));
  const altered = JSON.parse(await readFile(
    canonFile("vectors/invalid/altered-coherence-input-disclosure.json"),
    "utf8",
  ));
...
test("Node rejects the Python tampered receipt mutation", async () => {
  const receipt = JSON.parse(await readFile(
    canonFile("vectors/invalid/tampered-coherence-input-receipt.json"),
    "utf8",
  ));
